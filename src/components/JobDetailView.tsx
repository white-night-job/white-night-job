...
const StoreImagesGallery = dynamic(
    ...
    import("@/components/StoreImagesGallery").then((mod) => mod.StoreImagesGallery),
  {
    ssr: false,
    loading: () => (
      <div className="h-[150px] animate-pulse rounded-3xl border border-gold/15 bg-ivory/70" />
    ),
  },
);
...
function PreviewBlockedButton({
  ...
  const sizeClass =
    size === "lg"
      ? "apply-cta-btn apply-cta-btn--lg px-4 py-4 text-base sm:px-8 sm:text-lg"
      : size === "sm"
        ? "apply-cta-btn apply-cta-btn--sm px-4 py-2 text-sm"
        : "apply-cta-btn apply-cta-btn--md px-4 py-3 text-base sm:px-6";
  ...
  return (
    <button
      ...
      className={`${
        variant === "line"
          ? "apply-cta-btn--line"
          : `apply-cta-btn--phone ${className ?? luxuryBtnPrimary}`
      } ${sizeClass} ${fullWidth ? "w-full" : ""}`}
    >
      {variant === "line" ? (
        <svg
          className="apply-cta-icon"
          viewBox="0 0 24 24"
          fill="none"
          stroke="currentColor"
          aria-hidden
